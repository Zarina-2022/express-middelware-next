...
exports.updateTour = (req, res) => {
  ...
  */
  ...
  // tour ve json serveri update yapalim:
  const updatedData = req.body

  // guncellenecek id id'sini almak lazim
  const id = req.params.id

  // dizideki id'sini bilgigimiz elemanin updatedData icerisinde gelen degerlerini guncelle updatedData  icerisinde olmayan degerleride sabit birakmam gerekli
  ...
  fs.writeFile(`${__dirname}/../dev-data/data/tours-simple.json`, JSON.stringify(tours),(err) => {
    if (err) {
      // send it to frontend:
      res.status(404).json({
        status: 'guncelleme islemi basarisiz oldu'
      })
    } else {
      // send it to frontend:
      res.status(200).json({
...
exports.deleteTour = (req, res) => {
  ...
  */
  ...
  // In order for this change to be permanent, I also need to save it in the json file.
  fs.writeFile(`${__dirname}/../dev-data/data/tours-simple.json`, JSON.stringify(filteredTours), () => {
    // send response:
    res.status(204).send()
  })
}
